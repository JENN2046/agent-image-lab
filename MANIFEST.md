...
本包包含 Agent Image Lab release-readiness checkpoint 所需的文档、schema、角色定义、记忆策略、审片台规格、Adapter dry-run 包、VCP 接入记录、授权门和验收报告。

## 重要声明

- 包含 Adapter-only dry-run 包，不包含真实生图执行授权。
- 不包含真实图片文件。
- 不包含 API key、token、cookie 或私密路径。
