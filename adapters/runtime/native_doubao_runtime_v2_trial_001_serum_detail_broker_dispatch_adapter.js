...
const moduleId = "native_doubao_runtime_v2_trial_001_serum_detail_broker_dispatch_adapter";
const allowedPromptPackageRef = "prompts/image_generation/product_detail_premium_serum_bottle_v2.yaml";
const allowedOutputDirectory = "runs/real_generation/runtime_to_review_v2_trial_001_serum_detail_control/";
const requiredModel = "doubao-seedream-5-0-260128";
const routeTaskId = "AUTH-R2R-V2-TRIAL-001-SERUM-DETAIL-CONTROL-20260608-FUTURE-EXECUTION";
const pipelineId = "runtime_to_review_v2_trial_001_serum_detail_control";
const defaultRouteUrl = "http://127.0.0.1:6005/internal/ai-image-agents/execute/r2r-v2-trial-001-serum-detail-control";
const routePath = "/internal/ai-image-agents/execute/r2r-v2-trial-001-serum-detail-control";
const reviewBridgeRef = "review_console/live_receipt_bridge/r2r_v2_trial_001_serum_detail_control/bridge_entry.json";
const receiptRef = "reports/runtime_to_review_v2/r2r_v2_trial_001_serum_detail_control_receipt.json";
const artifactRecordRef = "reports/runtime_to_review_v2/r2r_v2_trial_001_serum_detail_control_artifact_record.json";
...
function failClosed(blocker, details = {}) {
  return {
    ...
    module_id: moduleId,
    status: "BLOCKED_R2R_V2_TRIAL_001_BROKER_DISPATCH_FAILED_CLOSED",
    blocker,
...
function inspectTrial001BrokerDispatchReadiness() {
  return {
    module_id: moduleId,
...
function routeRequestBody({ prompt, model, outputDirectory }) {
  return {
    ...
    activation: {
      ...
      confirmation_phrase: "RUNTIME_TO_REVIEW_V2_TRIAL_001_ONE_PROVIDER_ONE_IMAGE",
...
function postJson(urlString, body) {
  ...
  return new Promise((resolve) => {
    ...
    request.on("timeout", () => {
      request.destroy();
      resolve({ ok: false, statusCode: 0, body: { error: "r2r_v2_trial_001_broker_route_timeout" } });
    });
    request.on("error", () => {
      resolve({ ok: false, statusCode: 0, body: { error: "r2r_v2_trial_001_broker_route_unreachable" } });
    });
    request.end(payload);
  });
}
...
function routeBlocker(response) {
  if (!response || response.statusCode === 0) return "r2r_v2_trial_001_broker_route_unreachable";
  const body = response.body || {};
  const result = body.result || {};
  const baseBlocker = sanitizeRouteText(result.error || body.error || result.status) ||
    "r2r_v2_trial_001_broker_route_failed_closed";
...
async function realBoundOwnerRuntimeDelegate(request, options = {}) {
  ...
  if (issues.length > 0) return failClosed("r2r_v2_trial_001_secretless_request_invalid");
  if (request.output_directory_ref !== allowedOutputDirectory) {
    return failClosed("r2r_v2_trial_001_output_directory_not_allowed");
  }
  if (request.prompt_package_ref !== allowedPromptPackageRef) {
    return failClosed("r2r_v2_trial_001_prompt_package_not_allowed");
  }
  if (request.model !== requiredModel) {
    return failClosed("r2r_v2_trial_001_model_not_allowed", { model_sent: request.model || null });
  }
  ...
  try {
    ...
    return failClosed("r2r_v2_trial_001_prompt_package_invalid");
  }
  ...
  if (!outputRef) {
    return failClosed("r2r_v2_trial_001_output_file_missing", {
  ...
  try {
    ...
    return failClosed("r2r_v2_trial_001_output_file_invalid", {
      provider_contact_performed: true,
      plugin_call_performed: true,
      api_call_performed: true,
      image_generation_performed: true,
...
function createSecretlessProviderRuntime(options = {}) {
  return secretlessBridge.createBoundSecretlessProviderRuntimeBridge(
    ...
    {
      delegateOwner: "agent_image_lab_runtime_v2_trial_001_serum_detail_broker_dispatch_adapter",
...
module.exports.inspectTrial001BrokerDispatchReadiness = inspectTrial001BrokerDispatchReadiness;
