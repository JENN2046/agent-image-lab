...
const validator = "runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_010";
const activationPackageId = "AUTH-SECRETLESS-SERUM-LIVE-PROBE-20260603-010";
const requiredCommit = "39275a211964986b97fdb0d81119851353592071";
const routePath = "/internal/ai-image-agents/execute/serum-bottle-secretless";
const bindingPacketPath = "reports/runtime_to_review_v1/secretless_option_a_exact_binding_packet_draft_20260603_attempt_010.json";
const activationPreflightPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_activation_preflight_20260603_attempt_010.json";
const receiptPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_receipt_20260603_attempt_010.json";
const artifactPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_artifact_record_20260603_attempt_010.json";
const runnerPath = "scripts/run_runtime_to_review_v1_secretless_option_a_callable_runner.js";
const packageScriptName = "validate:runtime-to-review-secretless-serum-live-probe-receipt-attempt-010";
const manifestId = "runtime_to_review_secretless_serum_live_probe_receipt_attempt_010";
const outputDirectoryRef = "runs/real_generation/runtime_to_review_v1_guarded_live_probe_serum_bottle_secretless_attempt_010/";
...
function registrationChecks() {
  ...
  check("runner_exports_attempt_010_activation_and_defaults", () =>
    runner.exactRouteHttpActivationPackageIdAttempt010 === activationPackageId &&
    runner.exactRouteHttpPathAttempt005 === routePath &&
    runner.defaultRouteHttpInputAttempt010.activationPackageId === activationPackageId &&
    runner.defaultRouteHttpInputAttempt010.routeHttpPath === routePath &&
    runner.defaultRouteHttpInputAttempt010.routeHttpEndpointSource.includes(requiredCommit) &&
    runner.defaultRouteHttpInputAttempt010.receiptRef === receiptPath &&
    runner.defaultRouteHttpInputAttempt010.artifactRecordRef === artifactPath &&
    runner.defaultRouteHttpInputAttempt010.outputDirectoryRef === outputDirectoryRef &&
    runner.allowedExactRouteHttpActivationPackageIds.includes(activationPackageId)
  );

  check("runner_attempt_010_transport_preflight_validates", () => {
    ...
    return result.ok === true &&
      ...
      result.body.pipeline_id === "secretless-serum-live-probe-attempt-010" &&
  ...
  check("runner_cli_attempt_010_flag_binds_activation_defaults", () => {
    const input = runner.parseArgs([
      "--attempt-010-route-http",
      "--route-http-origin",
      "http://127.0.0.1:6005",
      "--confirmation-phrase",
  ...
  check("binding_packet_attempt_010_scope", () =>
    binding.packet_id === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-010" &&
    binding.status === "draft_not_active_non_executable" &&
    binding.exact_binding_packet_draft.binding_id === "secretless_option_a_serum_bottle_callable_binding_draft_20260603_010" &&
    ...
    binding.planned_attempt_010_outputs.receipt_ref === receiptPath &&
    binding.planned_attempt_010_outputs.artifact_record_ref === artifactPath
  );

  check("activation_preflight_attempt_010_scope", () =>
  ...
  check("package_script_registered", () =>
    packageJson.scripts &&
    packageJson.scripts[packageScriptName] === "node scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_010.js"
  );

  check("manifest_entry_registered_for_attempt_010", () => {
    const entry = manifest.validators.find((item) => item.id === manifestId);
    return entry &&
      entry.command === `npm run ${packageScriptName} -- --allow-pending` &&
      entry.script === "scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_010.js" &&
      entry.tier === "targeted" &&
...
function receiptChecks() {
  ...
  check("receipt_attempt_010_scope", () =>
  ...
  check("artifact_attempt_010_scope", () =>
