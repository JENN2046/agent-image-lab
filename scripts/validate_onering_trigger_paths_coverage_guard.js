...
const casesPath = "tests/schema_examples/onering_trigger_paths_coverage.example.json";
...
function main() {
  ...
  const commonRequired = fixture.common_required_trigger_paths || [];
  ...
  add("fixture_type_expected", fixture.fixture_type === "onering_trigger_paths_coverage");
  ...
  for (const entry of expectedEntries) {
    ...
    const triggerPaths = manifestEntry?.trigger_paths || [];
    const requiredTriggerPaths = [...commonRequired, ...(entry.required_trigger_paths || [])];
    const missing = missingValues(triggerPaths, requiredTriggerPaths);
    const absentFiles = requiredTriggerPaths.filter((triggerPath) => !fs.existsSync(repoPath(triggerPath)));

    add(`${entry.manifest_id}_entry_present`, Boolean(manifestEntry));
    add(`${entry.manifest_id}_trigger_paths_array`, Array.isArray(manifestEntry?.trigger_paths));
    add(`${entry.manifest_id}_required_trigger_paths_present`, missing.length === 0, {
      missing,
      required: requiredTriggerPaths,
      actual: triggerPaths,
    });
    add(`${entry.manifest_id}_script_trigger_path_matches_script_field`, triggerPaths.includes(manifestEntry?.script), {
      script: manifestEntry?.script,
      trigger_paths: triggerPaths,
    });
    add(`${entry.manifest_id}_required_trigger_files_exist`, absentFiles.length === 0, absentFiles);
  }
  ...
  const output = {
    ...
    validator: "validate_onering_trigger_paths_coverage_guard",
    cases: casesPath,
