...
const phase = "v0_6_27_exact_new_trial_003_shot_1_execution_closeout";
const payloadCapturePath = "reports/provider_payload_captures/v0_3_3_exact_new_trial_003_shot_1_request_payload.sanitized.json";
const attemptPath = "runs/real_generation/v0_3_3_exact_new_trial_003_shot_1/generation_attempt_result.json";
const receiptPath = "reports/provider_receipts/v0_3_3_exact_new_trial_003_shot_1_receipt.json";
const registryPath = "reports/provider_receipts/v0_3_3_exact_new_trial_003_shot_1_registry.json";
const bridgePath = "review_console/live_receipt_bridge/v0_3_3_exact_new_trial_003_shot_1/bridge_entry.json";
const reportPath = "reports/visual_asset_eval_dry_run/v0_6_27_exact_new_trial_003_shot_1_execution_closeout.json";
const passFixturePath = "tests/schema_examples/exact_new_trial_003_shot_1_execution_closeout.example.json";
const failFixturePath = "tests/schema_examples/exact_new_trial_003_shot_1_execution_closeout_fail.example.json";
const imagePath = "runs/real_generation/v0_3_3_exact_new_trial_003_shot_1/safe_adult_editorial_portrait_v1.png";
const expectedSha = "07a4ddc934c6e7ed88deefa9a1de6c8d06eb4407f4858f6688411dfa2bf60840";
...
function validateRecord(record, context) {
  ...
  assert(record.attempt_id === "v0_3_3_exact_new_trial_003_shot_1", `${context}.attempt_id mismatch`);
  ...
  assert(record.review_console_bridge_ref === "review_console/live_receipt_bridge/v0_3_3_exact_new_trial_003_shot_1", `${context}.review_console_bridge_ref mismatch`);
  ...
  assert(protocol.selected_shot_id === "v0_3_3_exact_new_trial_003_shot_1", `${context}.protocol.selected_shot_id mismatch`);
  ...
  assert(protocol.pre_provider_call_payload_capture_satisfied === false, `${context}.protocol.pre_provider_call_payload_capture_satisfied must be false`);
  assert(protocol.post_provider_call_payload_reconstruction_performed === true, `${context}.protocol.post_provider_call_payload_reconstruction_performed must be true`);
  ...
  assert(record.recommended_next === "prepare_shot_2_with_exact_pre_call_payload_capture_and_immediate_path_collision_recheck_before_any_auto_promotion_or_memory_write", `${context}.recommended_next mismatch`);
}
...
function validateNegativeCases(validRecord, invalidRecord) {
  ...
  const cases = [
    ...
    expectFailure(validRecord, "pretend_pre_call_capture", (candidate) => { candidate.protocol_compliance.pre_provider_call_payload_capture_satisfied = true; }),
    ...
    expectFailure(validRecord, "raw_payload_true", (candidate) => { candidate.boundary.raw_provider_payload_capture_performed = true; }),
    ...
    expectFailure(validRecord, "recommended_next_drift", (candidate) => { candidate.recommended_next = "promote_immediately"; }),
...
function main() {
  ...
  const report = readJson(reportPath).exact_new_trial_003_shot_1_execution_closeout;
  const passFixture = readJson(passFixturePath).exact_new_trial_003_shot_1_execution_closeout;
  const failFixture = readJson(failFixturePath).exact_new_trial_003_shot_1_execution_closeout;

  assert(payload.phase === phase, "payload phase mismatch");
  assert(payload.diagnostic_axes_capture_timing.pre_provider_call_capture_performed === false, "payload must record no pre-call capture");
  assert(payload.diagnostic_axes_capture_timing.post_provider_call_reconstruction_performed === true, "payload must record post-call reconstruction");
