...
const phase = "v0_6_28_exact_new_trial_003_shot_2_pre_call_payload_capture_preflight";
const expectedAttemptId = "v0_3_3_exact_new_trial_003_shot_2";
const expectedPrompt = "prompts/image_generation/safe_adult_editorial_portrait_v1.yaml";
const payloadCapturePath = "reports/provider_payload_captures/v0_3_3_exact_new_trial_003_shot_2_request_payload.sanitized.json";
const reportPath = "reports/visual_asset_eval_dry_run/v0_6_28_exact_new_trial_003_shot_2_pre_call_payload_capture_preflight.json";
const passFixturePath = "tests/schema_examples/exact_new_trial_003_shot_2_pre_call_payload_capture_preflight.example.json";
const failFixturePath = "tests/schema_examples/exact_new_trial_003_shot_2_pre_call_payload_capture_preflight_fail.example.json";
const shot2ExecutionCloseoutPath = "reports/visual_asset_eval_dry_run/v0_6_29_exact_new_trial_003_shot_2_execution_closeout.json";
const outputDirectory = "runs/real_generation/v0_3_3_exact_new_trial_003_shot_2/";
const outputImagePath = "runs/real_generation/v0_3_3_exact_new_trial_003_shot_2/safe_adult_editorial_portrait_v1.png";
const outputImageSha256 = "8bd7b81a916f0f6333392562d84e32368a3f28dd6a6456fc2f9e49d835a62c3b";
const attemptResultPath = "runs/real_generation/v0_3_3_exact_new_trial_003_shot_2/generation_attempt_result.json";
const receiptPath = "reports/provider_receipts/v0_3_3_exact_new_trial_003_shot_2_receipt.json";
const registryPath = "reports/provider_receipts/v0_3_3_exact_new_trial_003_shot_2_registry.json";
const bridgePath = "review_console/live_receipt_bridge/v0_3_3_exact_new_trial_003_shot_2/bridge_entry.json";
...
function validateCapture(capture) {
  ...
  assert(capture.payload_capture_id === "v0_3_3_exact_new_trial_003_shot_2_request_payload_sanitized", "capture id mismatch");
  ...
  assert(capture.diagnostic_axes_before_call.path_authorization === "003_shot_2_paths_absent_payload_captured_no_overwrite", "capture.path_authorization mismatch");
...
function validateRecord(record, context) {
  ...
  assert(record.recommended_next === "execute_v0_3_3_exact_new_trial_003_shot_2_with_pre_captured_payload_and_post_write_local_persistence_verification_before_any_auto_promotion_or_memory_write", `${context}.recommended_next mismatch`);
}
...
function shot2ExecutionCloseoutExists() {
  if (!exists(shot2ExecutionCloseoutPath)) return false;
  const reportRoot = readJson(shot2ExecutionCloseoutPath);
...
function main() {
  ...
  const executedAfterPreflight = shot2ExecutionCloseoutExists();
  if (executedAfterPreflight) {
    assert(exists(outputDirectory), "output directory must exist after shot_2 execution closeout");
    assert(sourceArtifactHashEvidence(outputImagePath, outputImageSha256).passed, "output image evidence must exist after shot_2 execution closeout");
    assert(exists(attemptResultPath), "attempt result must exist after shot_2 execution closeout");
    assert(exists(receiptPath), "receipt must exist after shot_2 execution closeout");
    assert(exists(registryPath), "registry must exist after shot_2 execution closeout");
    assert(exists("review_console/live_receipt_bridge/v0_3_3_exact_new_trial_003_shot_2"), "review console bridge directory must exist after shot_2 execution closeout");
  } else {
    assert(!exists(outputDirectory), "output directory must remain absent before shot_2 execution");
    assert(!exists(outputImagePath), "output image must remain absent before shot_2 execution");
    assert(!exists(attemptResultPath), "attempt result must remain absent before shot_2 execution");
    assert(!exists(receiptPath), "receipt must remain absent before shot_2 execution");
    assert(!exists(registryPath), "registry must remain absent before shot_2 execution");
    assert(!exists("review_console/live_receipt_bridge/v0_3_3_exact_new_trial_003_shot_2"), "review console bridge directory must remain absent before shot_2 execution");
  }

  const capture = readJson(payloadCapturePath);
  const report = readJson(reportPath).exact_new_trial_003_shot_2_pre_call_payload_capture_preflight;
  const passFixture = readJson(passFixturePath).exact_new_trial_003_shot_2_pre_call_payload_capture_preflight;
  const failFixture = readJson(failFixturePath).exact_new_trial_003_shot_2_pre_call_payload_capture_preflight;
