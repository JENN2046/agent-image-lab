...
const files = {
  phaseRecord: "docs/V0_6_40_EXACT_NEW_TRIAL_003_DAILY_NOTE_VCP_MEMORY_AUTHORIZATION_COMPILER_OUTPUT_PREFLIGHT.md",
  report: "reports/visual_asset_eval_dry_run/v0_6_40_exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight.json",
  passFixture: "tests/schema_examples/exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight.example.json",
  failFixture: "tests/schema_examples/exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight_fail.example.json",
...
const expected = {
  ...
  blockerDecision: "blocked_missing_accepted_sample_registration_archive_completion_production_candidate_authorization_memory_delta_and_daily_note_vcp_memory_authorization",
  packageStatus: "draft_blocked_missing_accepted_sample_registration_archive_completion_production_candidate_authorization_memory_delta_and_daily_note_vcp_memory_authorization",
  recommendedNext: "prepare_exact_new_trial_003_memory_delta_draft_package_with_daily_note_vcp_memory_dependency_preserved"
};
...
function evaluate(input) {
  ...
  const targetOk =
    ...
    productionCandidatePreflight.package_status === "draft_blocked_missing_accepted_sample_registration_archive_completion_and_production_candidate_authorization" &&
  ...
  const requiredOk =
    ...
    required.includes("exact Chinese memory_delta draft for the selected shot_2 route") &&
    required.includes("exact sensitive-data scan for the future DailyNote and VCP memory payload") &&
...
function validateNegativeCases(validRecord, invalidRecord) {
  ...
  const cases = [
    ...
    expectFailure(validRecord, "memory_delta_presence_overclaim_fails", (candidate) => {
      candidate.memory_delta_draft_present = true;
      candidate.target.memory_delta_draft_ref = "reports/provider_payload_captures/fake_memory_delta.yaml";
    }),
...
function main() {
  ...
  const report = readJson(files.report).exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight;
  const passFixture = readJson(files.passFixture).exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight;
  const failFixture = readJson(files.failFixture).exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight;
  ...
  assert(mvpText.includes("scripts/validate_exact_new_trial_003_daily_note_vcp_memory_authorization_compiler_output_preflight.js"), "validate_mvp missing new validator");
