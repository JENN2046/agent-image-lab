...
const casesPath = "tests/schema_examples/onering_manifest_tier_domain.example.json";
...
function main() {
  ...
  add("fixture_type_expected", fixture.fixture_type === "onering_manifest_tier_domain");
  add("expected_tier_targeted", fixture.expected_tier === "targeted", fixture.expected_tier);
  add("expected_domain_onering_contract", fixture.expected_domain === "onering_contract", fixture.expected_domain);
  ...
  for (const entry of expectedEntries) {
    ...
    const expectedTier = entry.tier || fixture.expected_tier;
    const expectedDomain = entry.domain || fixture.expected_domain;

    add(`${entry.manifest_id}_entry_present`, Boolean(manifestEntry));
    add(`${entry.manifest_id}_tier_matches`, manifestEntry?.tier === expectedTier, {
      expected: expectedTier,
      actual: manifestEntry?.tier,
    });
    add(`${entry.manifest_id}_domain_matches`, manifestEntry?.domain === expectedDomain, {
      expected: expectedDomain,
      actual: manifestEntry?.domain,
    });
  }
  ...
  const output = {
    ...
    validator: "validate_onering_manifest_tier_domain_guard",
    cases: casesPath,
