...
const validator = "runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_013";
const activationPackageId = "AUTH-SECRETLESS-SERUM-LIVE-PROBE-20260603-013";
const requiredCommit = "82b83028efaa2dcefa19edb03b6a8b3854941090";
const routePath = "/internal/ai-image-agents/execute/serum-bottle-secretless";
const bindingPacketPath = "reports/runtime_to_review_v1/secretless_option_a_exact_binding_packet_draft_20260603_attempt_013.json";
const activationPreflightPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_activation_preflight_20260603_attempt_013.json";
const receiptPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_receipt_20260603_attempt_013.json";
const artifactPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_artifact_record_20260603_attempt_013.json";
const runnerPath = "scripts/run_runtime_to_review_v1_secretless_option_a_callable_runner.js";
const packageScriptName = "validate:runtime-to-review-secretless-serum-live-probe-receipt-attempt-013";
const manifestId = "runtime_to_review_secretless_serum_live_probe_receipt_attempt_013";
const outputDirectoryRef = "runs/real_generation/runtime_to_review_v1_guarded_live_probe_serum_bottle_secretless_attempt_013/";
...
function registrationChecks() {
  ...
  check("runner_exports_attempt_013_activation_and_defaults", () =>
    runner.exactRouteHttpActivationPackageIdAttempt013 === activationPackageId &&
    runner.exactRouteHttpPathAttempt005 === routePath &&
    runner.defaultRouteHttpInputAttempt013.activationPackageId === activationPackageId &&
    runner.defaultRouteHttpInputAttempt013.routeHttpPath === routePath &&
    runner.defaultRouteHttpInputAttempt013.routeHttpEndpointSource.includes(requiredCommit) &&
    runner.defaultRouteHttpInputAttempt013.routeHttpEndpointSource.includes("1920x1920") &&
    runner.defaultRouteHttpInputAttempt013.receiptRef === receiptPath &&
    runner.defaultRouteHttpInputAttempt013.artifactRecordRef === artifactPath &&
    runner.defaultRouteHttpInputAttempt013.outputDirectoryRef === outputDirectoryRef &&
    runner.allowedExactRouteHttpActivationPackageIds.includes(activationPackageId)
  );

  check("runner_attempt_013_transport_preflight_validates", () => {
    ...
    return result.ok === true &&
      ...
      result.body.pipeline_id === "secretless-serum-live-probe-attempt-013" &&
  ...
  check("runner_cli_attempt_013_flag_binds_activation_defaults", () => {
    const input = runner.parseArgs([
      "--attempt-013-route-http",
      "--route-http-origin",
      "http://127.0.0.1:6005",
      "--confirmation-phrase",
  ...
  check("binding_packet_attempt_013_scope", () =>
    binding.packet_id === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-013" &&
    binding.status === "draft_not_active_non_executable" &&
    binding.exact_binding_packet_draft.binding_id === "secretless_option_a_serum_bottle_callable_binding_draft_20260603_012" &&
    ...
    binding.planned_attempt_013_outputs.receipt_ref === receiptPath &&
    binding.planned_attempt_013_outputs.artifact_record_ref === artifactPath
  );

  check("activation_preflight_attempt_013_scope", () =>
  ...
  check("package_script_registered", () =>
    packageJson.scripts &&
    packageJson.scripts[packageScriptName] === "node scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_013.js"
  );

  check("manifest_entry_registered_for_attempt_013", () => {
    const entry = manifest.validators.find((item) => item.id === manifestId);
    return entry &&
      entry.command === `npm run ${packageScriptName} -- --allow-pending` &&
      entry.script === "scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_013.js" &&
      entry.tier === "targeted" &&
...
function receiptChecks() {
  ...
  check("receipt_attempt_013_scope", () =>
  ...
  check("artifact_attempt_013_scope", () =>
