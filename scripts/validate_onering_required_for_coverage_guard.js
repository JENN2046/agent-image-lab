...
const casesPath = "tests/schema_examples/onering_required_for_coverage.example.json";
...
function main() {
  ...
  add("fixture_type_expected", fixture.fixture_type === "onering_required_for_coverage");
  ...
  for (const entry of expectedEntries) {
    ...
    const actualRequiredFor = manifestEntry?.required_for || [];
    const expectedRequiredFor = entry.required_for || [];
    const missing = missingValues(actualRequiredFor, expectedRequiredFor);

    add(`${entry.manifest_id}_entry_present`, Boolean(manifestEntry));
    add(`${entry.manifest_id}_required_for_array`, Array.isArray(manifestEntry?.required_for));
    add(`${entry.manifest_id}_required_for_values_present`, missing.length === 0, {
      missing,
      required: expectedRequiredFor,
      actual: actualRequiredFor,
    });
    add(`${entry.manifest_id}_required_for_non_empty`, actualRequiredFor.length > 0, actualRequiredFor.length);
  }
  ...
  const output = {
    ...
    validator: "validate_onering_required_for_coverage_guard",
    cases: casesPath,
