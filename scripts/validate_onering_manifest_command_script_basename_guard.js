...
const casesPath = "tests/schema_examples/onering_manifest_command_script_basename.example.json";
...
function commandTarget(command) {
  ...
  if (nodeIndex === -1 || nodeIndex + 1 >= parts.length) {
    return null;
  }
  return parts[nodeIndex + 1].replace(/\\/g, "/");
}
...
function main() {
  ...
  add("fixture_type_expected", fixture.fixture_type === "onering_manifest_command_script_basename");
  ...
  for (const entry of expectedEntries) {
    ...
    const commandPath = commandTarget(manifestEntry?.command);
    const commandBasename = commandPath ? path.posix.basename(commandPath) : null;
    const scriptBasename = manifestEntry?.script ? path.posix.basename(manifestEntry.script.replace(/\\/g, "/")) : null;

    add(`${entry.manifest_id}_entry_present`, Boolean(manifestEntry));
    add(`${entry.manifest_id}_command_has_node_script_target`, Boolean(commandPath), {
      command: manifestEntry?.command,
      command_target: commandPath,
    });
    add(`${entry.manifest_id}_command_basename_matches_script_basename`, commandBasename === scriptBasename, {
      command_basename: commandBasename,
      script_basename: scriptBasename,
    });
    add(`${entry.manifest_id}_basename_matches_fixture`, scriptBasename === entry.script_basename, {
      expected: entry.script_basename,
      actual: scriptBasename,
    });
  }
  ...
  const output = {
    ...
    validator: "validate_onering_manifest_command_script_basename_guard",
    cases: casesPath,
