...
const activationPackageId = "AUTH-SECRETLESS-SERUM-LIVE-PROBE-20260603-011";
const requiredCommit = "76ee3f2345d8fe490f6104bd0e670a5bebb99db2";
const routePath = "/internal/ai-image-agents/execute/serum-bottle-secretless";
const bindingPacketPath = "reports/runtime_to_review_v1/secretless_option_a_exact_binding_packet_draft_20260603_attempt_011.json";
const activationPreflightPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_activation_preflight_20260603_attempt_011.json";
...
const packageScriptName = "validate:runtime-to-review-secretless-serum-live-probe-receipt-attempt-011";
const manifestId = "runtime_to_review_secretless_serum_live_probe_receipt_attempt_011";
const outputDirectoryRef = "runs/real_generation/runtime_to_review_v1_guarded_live_probe_serum_bottle_secretless_attempt_011/";
...
function registrationChecks() {
  ...
  check("runner_exports_attempt_011_activation_and_defaults", () =>
    runner.exactRouteHttpActivationPackageIdAttempt011 === activationPackageId &&
    runner.exactRouteHttpPathAttempt005 === routePath &&
    runner.defaultRouteHttpInputAttempt011.activationPackageId === activationPackageId &&
    runner.defaultRouteHttpInputAttempt011.routeHttpPath === routePath &&
    runner.defaultRouteHttpInputAttempt011.routeHttpEndpointSource.includes(requiredCommit) &&
    runner.defaultRouteHttpInputAttempt011.routeHttpEndpointSource.includes("1920x1920") &&
    runner.defaultRouteHttpInputAttempt011.receiptRef === receiptPath &&
    runner.defaultRouteHttpInputAttempt011.artifactRecordRef === artifactPath &&
    runner.defaultRouteHttpInputAttempt011.outputDirectoryRef === outputDirectoryRef &&
    runner.allowedExactRouteHttpActivationPackageIds.includes(activationPackageId)
  );

  check("runner_attempt_011_transport_preflight_validates", () => {
    ...
    return result.ok === true &&
      ...
      result.body.pipeline_id === "secretless-serum-live-probe-attempt-011" &&
  ...
  check("runner_cli_attempt_011_flag_binds_activation_defaults", () => {
  ...
  check("binding_packet_attempt_011_scope", () =>
    binding.packet_id === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-011" &&
    ...
    binding.planned_attempt_011_outputs.receipt_ref === receiptPath &&
    binding.planned_attempt_011_outputs.artifact_record_ref === artifactPath
  );

  check("activation_preflight_attempt_011_scope", () =>
  ...
  check("package_script_registered", () =>
    packageJson.scripts &&
    packageJson.scripts[packageScriptName] === "node scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_011.js"
  );

  check("manifest_entry_registered_for_attempt_011", () => {
    const entry = manifest.validators.find((item) => item.id === manifestId);
    return entry &&
      entry.command === `npm run ${packageScriptName} -- --allow-pending` &&
      entry.script === "scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_011.js" &&
      entry.tier === "targeted" &&
...
function receiptChecks() {
  ...
  check("receipt_attempt_011_scope", () =>
  ...
  check("artifact_attempt_011_scope", () =>
