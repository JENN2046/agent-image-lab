...
const validator = "runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_009";
const activationPackageId = "AUTH-SECRETLESS-SERUM-LIVE-PROBE-20260603-009";
const requiredCommit = "32e5c2a7de9edb7e243671a5a18b517caafc8645";
const routePath = "/internal/ai-image-agents/execute/serum-bottle-secretless";
const bindingPacketPath = "reports/runtime_to_review_v1/secretless_option_a_exact_binding_packet_draft_20260603_attempt_009.json";
const activationPreflightPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_activation_preflight_20260603_attempt_009.json";
const receiptPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_receipt_20260603_attempt_009.json";
const artifactPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_artifact_record_20260603_attempt_009.json";
const runnerPath = "scripts/run_runtime_to_review_v1_secretless_option_a_callable_runner.js";
const packageScriptName = "validate:runtime-to-review-secretless-serum-live-probe-receipt-attempt-009";
const manifestId = "runtime_to_review_secretless_serum_live_probe_receipt_attempt_009";
const outputDirectoryRef = "runs/real_generation/runtime_to_review_v1_guarded_live_probe_serum_bottle_secretless_attempt_009/";
...
function registrationChecks() {
  ...
  check("runner_exports_attempt_009_activation_and_defaults", () =>
    runner.exactRouteHttpActivationPackageIdAttempt009 === activationPackageId &&
    runner.exactRouteHttpPathAttempt005 === routePath &&
    runner.defaultRouteHttpInputAttempt009.activationPackageId === activationPackageId &&
    runner.defaultRouteHttpInputAttempt009.routeHttpPath === routePath &&
    runner.defaultRouteHttpInputAttempt009.routeHttpEndpointSource.includes(requiredCommit) &&
    runner.defaultRouteHttpInputAttempt009.receiptRef === receiptPath &&
    runner.defaultRouteHttpInputAttempt009.artifactRecordRef === artifactPath &&
    runner.defaultRouteHttpInputAttempt009.outputDirectoryRef === outputDirectoryRef &&
    runner.allowedExactRouteHttpActivationPackageIds.includes(activationPackageId)
  );

  check("runner_attempt_009_transport_preflight_validates", () => {
    ...
    return result.ok === true &&
      ...
      result.body.pipeline_id === "secretless-serum-live-probe-attempt-009" &&
  ...
  check("runner_cli_attempt_009_flag_binds_activation_defaults", () => {
    const input = runner.parseArgs([
      "--attempt-009-route-http",
      "--route-http-origin",
      "http://127.0.0.1:6005",
      "--confirmation-phrase",
  ...
  check("binding_packet_attempt_009_scope", () =>
    binding.packet_id === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-009" &&
    ...
    binding.planned_attempt_009_outputs.receipt_ref === receiptPath &&
    binding.planned_attempt_009_outputs.artifact_record_ref === artifactPath
  );

  check("activation_preflight_attempt_009_scope", () =>
  ...
  check("package_script_registered", () =>
    packageJson.scripts &&
    packageJson.scripts[packageScriptName] === "node scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_009.js"
  );

  check("manifest_entry_registered_for_attempt_009", () => {
    const entry = manifest.validators.find((item) => item.id === manifestId);
    return entry &&
      entry.command === `npm run ${packageScriptName} -- --allow-pending` &&
      entry.script === "scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_009.js" &&
      entry.tier === "targeted" &&
...
function receiptChecks() {
  ...
  check("receipt_attempt_009_scope", () =>
  ...
  check("artifact_attempt_009_scope", () =>
