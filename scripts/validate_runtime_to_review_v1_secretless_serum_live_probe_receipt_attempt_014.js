...
const validator = "runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_014";
const activationPackageId = "AUTH-SECRETLESS-SERUM-LIVE-PROBE-20260603-014";
const requiredCommit = "549a26abc7d34e973c9d1ac6d4491aa8d92e88f1";
const routePath = "/internal/ai-image-agents/execute/serum-bottle-secretless";
const bindingPacketPath = "reports/runtime_to_review_v1/secretless_option_a_exact_binding_packet_draft_20260603_attempt_014.json";
const activationPreflightPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_activation_preflight_20260603_attempt_014.json";
const promptPackagePath = "reports/runtime_to_review_v1/secretless_serum_attempt_014_prompt_quality_package_20260604.json";
const receiptPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_receipt_20260603_attempt_014.json";
const artifactPath = "reports/runtime_to_review_v1/secretless_serum_live_probe_artifact_record_20260603_attempt_014.json";
const runnerPath = "scripts/run_runtime_to_review_v1_secretless_option_a_callable_runner.js";
const packageScriptName = "validate:runtime-to-review-secretless-serum-live-probe-receipt-attempt-014";
const manifestId = "runtime_to_review_secretless_serum_live_probe_receipt_attempt_014";
const outputDirectoryRef = "runs/real_generation/runtime_to_review_v1_guarded_live_probe_serum_bottle_secretless_attempt_014/";
...
function registrationChecks() {
  ...
  const promptPackage = readJson(promptPackagePath);
  const runner = require(repoPath(runnerPath));
  const runnerSource = fs.readFileSync(repoPath(runnerPath), "utf8");

  check("runner_exports_attempt_014_activation_and_defaults", () =>
    runner.exactRouteHttpActivationPackageIdAttempt014 === activationPackageId &&
    runner.exactRouteHttpPathAttempt005 === routePath &&
    runner.defaultRouteHttpInputAttempt014.activationPackageId === activationPackageId &&
    runner.defaultRouteHttpInputAttempt014.routeHttpPath === routePath &&
    runner.defaultRouteHttpInputAttempt014.routeHttpEndpointSource.includes(requiredCommit) &&
    runner.defaultRouteHttpInputAttempt014.routeHttpEndpointSource.includes("1920x1920") &&
    runner.defaultRouteHttpInputAttempt014.routeHttpEndpointSource.includes("sha256/mime/dimensions") &&
    runner.defaultRouteHttpInputAttempt014.prompt.includes("complete full bottle fully visible") &&
    runner.defaultRouteHttpInputAttempt014.prompt.includes("generous bottom margin") &&
    runner.defaultRouteHttpInputAttempt014.prompt.includes("no cropping") &&
    runner.defaultRouteHttpInputAttempt014.receiptRef === receiptPath &&
    runner.defaultRouteHttpInputAttempt014.artifactRecordRef === artifactPath &&
    runner.defaultRouteHttpInputAttempt014.outputDirectoryRef === outputDirectoryRef &&
    runner.allowedExactRouteHttpActivationPackageIds.includes(activationPackageId)
  );

  check("runner_attempt_014_transport_preflight_validates", () => {
    ...
    return result.ok === true &&
      ...
      result.body.pipeline_id === "secretless-serum-live-probe-attempt-014" &&
      result.body.plan.steps[0].plugin === "DoubaoGen" &&
      result.body.plan.steps[0].type === "generate_image" &&
      result.body.plan.steps[0].prompt.includes("complete full bottle fully visible") &&
      result.body.plan.steps[0].prompt.includes("generous bottom margin") &&
      result.body.plan.steps[0].prompt.includes("no cropping") &&
      result.body.plan.steps[0].output_directory_ref === outputDirectoryRef &&
  ...
  check("runner_cli_attempt_014_flag_binds_activation_defaults", () => {
    const input = runner.parseArgs([
      "--attempt-014-route-http",
      "--route-http-origin",
      "http://127.0.0.1:6005",
      "--confirmation-phrase",
  ...
  check("binding_packet_attempt_014_scope", () =>
    binding.packet_id === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-014" &&
    binding.status === "draft_not_active_non_executable" &&
    binding.exact_binding_packet_draft.binding_id === "secretless_option_a_serum_bottle_callable_binding_draft_20260603_014" &&
    ...
    binding.exact_binding_packet_draft.vcptoolbox_required_controls.native_delegate_fixed_size === "1920x1920" &&
    binding.exact_binding_packet_draft.vcptoolbox_required_controls.pipeline_safety_gate.includes("allowExecutionWithoutEnvGate") &&
    binding.planned_attempt_014_outputs.receipt_ref === receiptPath &&
    binding.planned_attempt_014_outputs.artifact_record_ref === artifactPath
  );

  check("activation_preflight_attempt_014_scope", () =>
    ...
    preflight.vcptoolbox_fixed_payload_controls.native_delegate_fixed_size === "1920x1920" &&
    preflight.vcptoolbox_fixed_payload_controls.size_override_allowed_from_agent_image_lab === false &&
  ...
  check("prompt_package_attempt_014_quality_controls", () =>
    promptPackage.activation_package_id_future === activationPackageId &&
    promptPackage.binding_packet_id_future === "BINDING-DRAFT-SECRETLESS-OPTION-A-SERUM-20260603-014" &&
    promptPackage.prompt === runner.defaultRouteHttpInputAttempt014.prompt &&
    promptPackage.prompt.includes("complete full bottle fully visible") &&
    promptPackage.prompt.includes("generous bottom margin") &&
    promptPackage.prompt.includes("no cropping") &&
    promptPackage.non_execution_boundary.route_http_request_performed === false &&
    promptPackage.non_execution_boundary.provider_contact_performed === false
  );

  check("package_script_registered", () =>
    packageJson.scripts &&
    packageJson.scripts[packageScriptName] === "node scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_014.js"
  );

  check("manifest_entry_registered_for_attempt_014", () => {
    const entry = manifest.validators.find((item) => item.id === manifestId);
    return entry &&
      entry.command === `npm run ${packageScriptName} -- --allow-pending` &&
      entry.script === "scripts/validate_runtime_to_review_v1_secretless_serum_live_probe_receipt_attempt_014.js" &&
      entry.tier === "targeted" &&
...
function receiptChecks() {
  ...
  check("receipt_attempt_014_scope", () =>
