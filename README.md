...
```yaml
current_phase: runtime_to_review_minimal_real_loop_v1_owner_runtime_binding_contract
...
receipt_registry_active: true
cost_rollback_schema_hardened: true
...
current_next_boundary: guarded_live_probe_execution_requires_delegate_and_exact_confirmation
...
current_local_capability_completed: retry_007_closed_provider_link_evidence_only_runtime_v1_guarded_skeleton_readonly_real_review_session_decision_record_draft_registry_fixture_smoke_flow_validation_gate_split_static_real_entry_viewer_native_doubao_delegate_module_and_owner_runtime_binding_contract
current_local_capability_completed_lane: Green_local_owner_runtime_binding_contract_no_real_provider_call
current_local_capability_next: guarded_live_probe_execution_with_real_bound_owner_runtime
current_local_capability_next_lane: Amber_B_budgeted_provider_image_with_receipt_exact_confirmation_real_bound_owner_runtime_and_no_secret_value_read
...
recommended_next_if_continuing_local_product_work: guarded_live_probe_execution_with_real_bound_owner_runtime
recommended_next: runtime_to_review_minimal_real_loop_v1_guarded_live_probe_execution_with_real_bound_owner_runtime
```

Current state orientation lives in `CURRENT_STATE.md`, with command risk tiers
in `docs/COMMAND_TIERS.md`. Readiness language in this README is scoped:
`local_full_autopilot_ready` means local governance and validation readiness,
not unrestricted provider execution, image generation, memory write, production
promotion, push, tag, release, or deploy.
...
Runtime-to-review mainline status: retry 007 is closed as provider link success evidence only and must not be promoted to accepted sample or production candidate. `runtime_kernel_v1_real_provider_guarded` now starts the product mainline beside the v0 stub: fixture mode produces canonical artifact/audit/review metadata, real-guarded mode fails closed unless a structured provider delegate is injected, `review_bridge_runtime_v1_readonly` turns runtime v1 bridge metadata into a readonly Review Console session without reading image binaries, `review_decision_record_v1` persists metadata-only human decisions with the fixed five-value enum, and `review_draft_registry_v1` generates accepted/rejected/rework draft records while keeping retry 007 at `no_registry_draft`. `run_runtime_to_review_v1_fixture_smoke_flow.js` now runs the no-provider fixture chain from prompt fixture through runtime, artifact, audit, review bridge, readonly session, request-rework decision, and rework draft registry metadata. Phase 7 separates validation into `validate:runtime-to-review-default-local`, `validate:runtime-to-review-evidence`, and `validate:runtime-to-review-guarded-live-probe-gate`; the explicit live runner is `runtime-to-review:guarded-live-probe`. `adapters/runtime/native_doubao_runtime_v1_provider_delegate.js` is the NativeDoubao runtime v1 provider delegate module: it validates the runtime v1 delegate request, maps only to the secretless controlled bridge path, fails closed when no bound owner runtime exists, and never uses the legacy `.env.local` secret-reading path. `scripts/run_runtime_to_review_v1_guarded_live_probe.js` now accepts `--owner-runtime-module`, and `adapters/runtime/native_doubao_runtime_v1_owner_runtime_binding_contract.js` proves the binding path with a no-provider owner-runtime contract: exact phrase + delegate + owner runtime module passes preflight, but the no-provider contract fails closed and claims no live provider success. Validators still perform no real provider/plugin/API/image action. `review_console/static_prototype/runtime_v1_real_entry_viewer.html` is the separate readonly real-entry Review Console mode; it loads `runtime_v1_real_entry_session.js` plus `runtime_v1_real_entry_viewer.js`, not `mock_data.js`, and renders runtime v1 session metadata without fetch, image binary read, file write, provider/plugin/API call, memory write, or production write. Default validation performs no real provider/plugin/API/image action.
...
`docs/AUTOPILOT_GOAL_DECOMPOSITION_RUNTIME.md` defines the runtime rule for non-single-step goals: compile `goal -> route_plan -> executable task_queue -> blocked_red_items -> next_safe_task`, execute only `next_safe_task`, validate, update `.agent_board`, record an Amber receipt when needed, and continue until done, blocked, or Red.

`scripts/materialize_autopilot_goal_decomposition.js` is the local dry-run materializer for that runtime. It reads the runtime example and emits a deterministic materialized snapshot; it is not a real executor and does not call provider/plugin/API/image/memory/source-read/runtime/dependency actions.
...
`scripts/validate_autopilot_false_readiness_negative_cases.js` hardens the
chain against false readiness claims. It generates local negative cases for
ambiguous next-task fields, fixture/current-boundary confusion, hidden queue
...
False-readiness hardening status: the current selected Green task is
`add_false_readiness_negative_case_validator`. Other candidate gaps remain
queued or blocked: `receipt_registry_negative_cases_v1` is the next local
...
- Receipt registry: `tests/schema_examples/autopilot_receipt_registry.example.json`
...
- `docs/archive/DOCS_ARCHIVE_C1H_REMAINING_ARCHIVE_ROUTE_DECISION_DRY_RUN.md` 记录 C1f 后剩余 archive 路线裁决；它确认当前没有剩余 exact-move safe candidates，并建议转入 docs-only-reference link graph dry-run。
- `docs/archive/DOCS_ARCHIVE_C1I_DOCS_ONLY_REFERENCE_LINK_GRAPH_DRY_RUN.md` 记录 67 个 docs-only-reference 的当前引用图；它确认 65 个仍需引用改写策略、2 个变成零引用候选。
...
- `docs/archive/DOCS_ARCHIVE_C1AJ_AGENT_BOARD_ONLY_01_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AK_AGENT_BOARD_ONLY_01_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第一批 agent-board-only 20 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_02_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_02_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第二批 agent-board-only 20 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_03_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_03_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第三批 agent-board-only 20 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_04_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_04_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第四批 agent-board-only 20 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_05_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_ONLY_05_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第五批 agent-board-only 13 文件 paired move/rewrite 执行，并收口 agent-board-only lane。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_01_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_01_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第一批 agent-board-plus-docs 12 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_02_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_02_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第二批 agent-board-plus-docs 12 文件 paired move/rewrite 执行。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_03_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_03_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第三批 agent-board-plus-docs 19 文件 paired move/rewrite 执行，含 4 个确认零引用移动。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_04_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_04_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第四批 agent-board-plus-docs 20 文件 paired move/rewrite 执行，含 7 个确认零引用移动。
- `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_05_EXECUTION_PACKAGE.md` 和 `docs/archive/DOCS_ARCHIVE_C1AM_AGENT_BOARD_PLUS_DOCS_05_EXECUTION_RECORD.md` 记录 C1 wrapper-required 第五批 agent-board-plus-docs 5 文件 paired move/rewrite 执行，并收口低风险 agent-board-plus-docs lane。
...
- `docs/P5D_FAILURE_SAMPLE_CAPSULE_NEGATIVE_CASE_COVERAGE.md` 记录 failure sample capsule validator 负例覆盖；fixture 仅在 `.agent_private/`，不改真实 failure samples。
- `docs/P5E_FAILURE_SAMPLE_VALIDATORS_MVP_WIRING.md` 记录 failure sample validators 纳入 `scripts/validate_mvp.ps1`；当前零样本状态仍然通过。
- `docs/P5F_FIRST_FAILURE_SAMPLE_CAPSULE_AUTHORIZATION_PACKAGE_DRY_RUN.md` 记录第一颗 failure sample capsule 授权包 dry-run；推荐 `failure_french_summer_rattan_bag_v7_29_001`，但不创建 capsule、不创建 preview。
