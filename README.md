...
v7.67 Cancel Preflight Endpoint Lock and Execution Authorization Gate — docs-only endpoint lock and authorization gate. Exact endpoint locked to remote-debug CDP strategy: window.imageLabReview.cancel({}) via Runtime.evaluate. Cancel payload locked to {}. Max 1 call, no retry, no fallback. loadSession, previewDraft, submitDraft permanently forbidden. Execution not authorized. Authorization phrase: "批准 v7.67 cancel preflight".
```
