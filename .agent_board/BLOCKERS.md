...
Current active blockers are BLOCKER-20260601-01 for the historical admin-auth route-owner runtime path and BLOCKER-20260603-01 narrowed for the secretless Option A route: attempt-005 reached the independent internal route and failed closed with `serum_bottle_secretless_plugin_manager_missing`; VCPToolBox was then locally refreshed to d0d5c104 with pluginManager/internal authorizer injection, and Agent Image Lab now has a pending attempt-006 runner/validator refresh. Route/live success still requires validation, exact-file commit, and a new exact activation before any attempt-006 POST. Resolved/legacy entries below are historical unless marked active.
