...
Status: narrowed_by_readonly_binding_verification_router_refs_not_bound
...
Required next safe action: draft or authorize an exact VCPToolBox router binding patch/preflight limited to enabling the secretless route gate and binding a non-secret internal authorizer, then verify from a clean main checkout before any route HTTP request. Do not retry from attempt_001.
