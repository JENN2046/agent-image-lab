...
```text
Agent Image Lab is in v14_076_review_report_production_exclusion_register_gate. This local fixture validator turns ReviewReport never-production decisions into auditable production exclusion records while all memory writes, production writes, accepted_samples writes, provider execution, plugin/API/image generation, deployment, release, and VCP source access remain blocked.
```
