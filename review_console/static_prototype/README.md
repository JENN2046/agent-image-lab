...
Readonly Visual Review MVP 新增 `readonly_visual_review_mvp_state`：把 P3.22 的 24 个 readonly review artifacts、pass / patch / reject 审查行、taxonomy tags、metadata sections、next actions 和 guard summary 接到静态 Review Console。它只使用 `mock_data.js` / fixture seed 和浏览器内存草案，不读取 `asset_archive/`，不加载图片，不 fetch，不写文件，不调用 provider / plugin / API / DailyNote / VCP memory，也不写 accepted_samples 或 production_candidate。
Readonly Visual Review Dataset Regression 新增 `readonly_visual_review_dataset_regression_state`：在只读工作台下扩展 7 个视觉评审维度 x pass / patch / reject 的 21 行矩阵，`needs_revision` 只作为 `patch` 的 alias，不新增 outcome enum，也不改变 P3.22 24-artifact catalog 计数。它只使用 `mock_data.js` / fixture seed 和浏览器内存草案，不读取 `asset_archive/`，不加载图片，不 fetch，不写文件，不调用 runtime、provider / plugin / API / DailyNote / VCP memory，也不写 accepted_samples 或 production_candidate。
...
- `readonly_visual_review_dataset_regression_state`
...
`readonly_visual_review_mvp_state` 必须作为本地静态只读审查工作台呈现；它只显示已有 artifact catalog、readonly corpus renderer 和 Review Console static handoff 的派生摘要，不代表浏览器执行 validator、读取 `asset_archive/`、加载图片、fetch、写文件、provider/plugin/API 调用、图片生成、accepted_samples 写入、production_candidate 晋级或记忆写入。
`readonly_visual_review_dataset_regression_state` 必须作为本地静态只读数据集回归层呈现；它只显示 7 个视觉评审维度、21 行 pass / patch / reject 矩阵、taxonomy coverage、metadata sections、next actions 和 guard summary，不代表浏览器执行 validator、读取 `asset_archive/`、加载图片、fetch、写文件、runtime、provider/plugin/API 调用、图片生成、accepted_samples 写入、production_candidate 晋级或记忆写入。
...
`P5L_REVIEW_CONSOLE_FAILURE_CAPSULE_SNAPSHOT.example.json` 必须与 P5K failure capsule 静态显示保持一致；它固定 sample id、512 WebP preview metadata、clone-portable status、failure tags、resolved accepted sample 和 never-production route，不得读取 `asset_archive/`、加载 preview、写 failure_samples、写 memory 或声称 VCP runtime integration。
