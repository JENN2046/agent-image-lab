# Real VCPChat Execute-once Authorization Preplan Contract

本文定义真实 VCPChat 一次性读取执行授权前规划的 contract。它只描述未来执行一次只读读取前的授权包结构，不读取真实源码，不保存真实路径，不保存真实命令，不修改真实 VCPChat，不创建 IPC handler，不调用插件、API、DailyNote 或文件系统。

## Contract Status

```yaml
contract:
  name: real_vcpchat_execute_once_authorization_preplan
  version: v2.3-execute-once-preplan-template
...
真实路径、读取命令、源码原文、endpoint 原文、环境变量值、secret、客户隐私和 runtime log 原文不得进入 packet shape。

## Authorization Preconditions

```yaml
authorization_preconditions:
  required_before_authorization_request:
    - v2_2_authorization_fill_exists
    - v2_2_filled_authorization_review_gate_exists
    - v2_2_final_authorization_preflight_exists
    - v2_2_one_time_read_authorization_request_exists
    - v2_2_execution_preflight_exists
...
本阶段样例不得把 `execute_once_authorization_granted` 设为 `true`。未来即使授权包获批，也只允许进入下一独立执行请求，不得自动读取源码。
