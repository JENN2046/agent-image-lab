# Execute-once Command Binding Preflight Contract

本文定义真实 VCPChat 一次性读取命令绑定 preflight 的 contract。它只描述未来读取命令绑定前的检查记录形态，不读取真实源码，不保存真实路径，不保存真实命令，不修改真实 VCPChat，不创建 IPC handler，不调用插件、API、DailyNote 或文件系统。

## Contract Status

```yaml
contract:
  name: execute_once_command_binding_preflight
  version: v2.3-command-binding-preflight-template
...
binding_preconditions:
  required_before_binding:
    - execute_once_authorization_request_exists
...
本阶段样例不得把 `command_binding_preflight_passed` 设为 `true`。未来即使命令绑定 preflight 通过，也只允许进入下一独立最终读取授权门，不得自动读取源码。

## Forbidden Binding Content

```yaml
forbidden_binding_content:
  - real_local_path
  - raw_read_command
  - shell_command_text
  - raw_source_code
...
## Boundary After Binding Preflight

命令绑定 preflight 完成后也不得自动进入：
...
- 本 contract 只定义一次性读取命令绑定 preflight。
- 本 contract 不执行真实读取。
- 本 contract 不保存真实路径。
