...
7. Use `runtime_to_review_v2_formal_v11_live_activation_packet_checklist` to issue a separate exact activation packet only after owner authorization; the checklist itself is not authorization.
8. Execute `runtime-to-review:guarded-live-probe` only with `RUNTIME_TO_REVIEW_V1_ONE_PROVIDER_ONE_IMAGE`, the real bound owner runtime module, an explicit owner-provided VCPToolBox root, one provider/plugin/API call, one image max, and receipt/status sync.
9. Start production workflow only after provider output can enter artifact / review / audit and human decision records safely.
