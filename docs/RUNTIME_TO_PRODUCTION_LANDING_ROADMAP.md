...
7. Execute `runtime-to-review:guarded-live-probe` only with `RUNTIME_TO_REVIEW_V1_ONE_PROVIDER_ONE_IMAGE`, the real bound owner runtime module, an explicit owner-provided VCPToolBox root, one provider/plugin/API call, one image max, and receipt/status sync.
8. Start production workflow only after provider output can enter artifact / review / audit and human decision records safely.
