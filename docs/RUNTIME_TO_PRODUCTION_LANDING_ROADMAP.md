...
5. Review `runtime_to_review_v2_review_feedback_prompt_patch_preview`; if accepted, first select a fresh non-colliding prompt package target because the original v3 target already exists and must not be overwritten.
6. Execute `runtime-to-review:guarded-live-probe` only with `RUNTIME_TO_REVIEW_V1_ONE_PROVIDER_ONE_IMAGE`, the real bound owner runtime module, an explicit owner-provided VCPToolBox root, one provider/plugin/API call, one image max, and receipt/status sync.
7. Start production workflow only after provider output can enter artifact / review / audit and human decision records safely.
