...
- 这仍不是 production runtime；当前只证明了 runtime-to-review metadata envelope、fail-closed delegate boundary、readonly review session read path、metadata-only human decision record、draft registry metadata route、no-provider fixture smoke flow 和三层 validation gate split。
