...
- `runtime_to_review_v2_visual_eval_min_repeatable_fixture_set` 已补一个 Green metadata-only fixture set：它把 `docs/VISUAL_EVAL_RUBRIC.md` 的 `pass` / `patch` / `reject`、0-5 分、必填 review notes 和下一步 route action 固定成三条可重复样例，连接 Trial 002 tracked decision 与 synthetic patch/reject routing；它不读图像二进制、不执行 provider、不写 accepted_samples、memory 或 production。
- `runtime_to_review_v2_review_feedback_prompt_patch_preview` 已补一个 Green metadata-only preview：它把 Trial 002 accepted review 的 watch items 转成下一轮 prompt package 候选 delta，明确 cleaner SKU 背景、降低前景桌面主导、保留 side control / handle 几何和 lower body detail；它不创建或覆盖任何 prompt 文件、不执行 provider、不读图、不写 memory 或 production。后续检查发现 preview 原候选目标 `product_lifestyle_premium_portable_led_camping_lantern_v3.yaml` 已是既有 tracked prompt package，因此正式写入前必须重新选择非冲突 target。
- `runtime_to_review_v2_review_feedback_fresh_prompt_target_selection` 已补一个 Green metadata-only target selection：它把正式写入目标从既有 tracked v3 改为 fresh target `prompts/image_generation/product_lifestyle_premium_portable_led_camping_lantern_v11.yaml`，证明 v3 冲突、v11 不覆盖既有 prompt package，并继续保持 selection 阶段的 prompt file write/provider/image/memory/production 全部 false。
- `runtime_to_review_v2_formal_v11_prompt_package` 已补一个 Green exact-file prompt package：`prompts/image_generation/product_lifestyle_premium_portable_led_camping_lantern_v11.yaml` 从 Trial 002 accepted watch items、prompt patch preview、fresh target selection 生成 clean SKU/product-first prompt，显式保留 scene hero route separate，并声明 no provider, image, memory, production, or accepted_samples write；它不是 live probe、A5 execution、memory write 或 production promotion 授权。
- `runtime_to_review_v2_formal_v11_review_criteria_preflight_gate` 已补一个 Green metadata-only review criteria/preflight gate：`reports/runtime_to_review_v2/r2r_v2_formal_v11_review_criteria_preflight_gate_20260616.json` 把 formal v11 watch items 固定成 pass/patch/reject criteria、pre-live stop conditions 和 one-provider/one-image 预算边界；live probe 仍需 separate exact authorization，且本 gate 不执行 provider、不读图、不写 memory/production/archive/accepted_samples。
- `runtime_to_review_v2_formal_v11_live_activation_packet_checklist` 已补一个 Green metadata-only activation checklist：`reports/runtime_to_review_v2/r2r_v2_formal_v11_live_activation_packet_checklist_20260616.json` 把 formal v11 gate 到 guarded live probe exact activation packet 的 preflight / receipt 核对项固定下来；它仍然 `can_execute_now=false`，不授权 live probe，不执行 provider/API/image，不写 output、accepted_samples、production、archive 或 memory。
- `runtime_to_review_v2_formal_v11_guarded_live_activation_packet_draft` 已补一个 Green inactive activation packet draft：`reports/runtime_to_review_v2/r2r_v2_formal_v11_guarded_live_activation_packet_draft_20260616.json` 和 `tests/fixtures/runtime_kernel_v2_formal_v11_lantern_sku_task.fixture.json` 把 formal v11 prompt、review gate、activation checklist、one-provider/one-image 预算、receipt refs、v11 output scope 和 owner-runtime scope alignment 要求固定到一个草案；它不翻转 `can_execute_now`，不授权 provider/image/output/memory/production。
...
- Runtime v1 fixture result 可以走到 `completed_fixture_artifact`，real-guarded 缺少 delegate、缺少 owner runtime module、或 owner runtime contract 没有真实 provider 时走到 `failed_closed`，validator 使用 fake delegate/runner 或 no-provider owner runtime contract 只验证结构，不触发真实 provider/plugin/API/image action；Review bridge v1 validator 能把 fixture/fake-provider bridge entry 打开为 readonly real session；decision record 和 draft registry validators 能写入并清理本地 metadata-only probe；fixture smoke validator 能验证完整 no-provider runtime-to-review metadata chain，并捕获 provider failure、model mismatch、invalid MIME metadata、invalid review decision、forbidden production flag、forbidden memory write；Phase 7 gate validator 能证明默认本地验证不包含 live probe，evidence validation 只检查既有证据，live probe runner 缺少 delegate/精确确认时不执行，错误精确短语会阻断。
- Red fixture 可以在 policy gate 进入 `blocked_red`，executor 不运行。
- audit write and durable audit store 已进入受控本地 `.agent_private` 路线。
- 这仍不是 production runtime；当前只证明了 runtime-to-review metadata envelope、fail-closed NativeDoubao delegate module、owner runtime binding injection contract、精确确认短语 preflight gate、readonly review session read path、metadata-only human decision record、draft registry metadata route、no-provider fixture smoke flow、三层 validation gate split 和独立 Review Console static real-entry viewer。
...
4. Use `runtime_to_review_v2_visual_eval_min_repeatable_fixture_set` as the minimum repeatable review evidence before selecting any fresh prompt target.
5. Use `runtime_to_review_v2_review_feedback_fresh_prompt_target_selection` as the target guard: the prompt path is `prompts/image_generation/product_lifestyle_premium_portable_led_camping_lantern_v11.yaml`, and the existing tracked v3 package must not be overwritten.
6. Review `runtime_to_review_v2_formal_v11_review_criteria_preflight_gate` before any live probe; it carries the formal v11 watch items but does not authorize execution.
7. Review `runtime_to_review_v2_formal_v11_guarded_live_activation_packet_draft`; it is inactive and cannot execute, but it fixes the exact future packet shape.
8. Use `runtime_to_review_v2_formal_v11_live_activation_packet_checklist` to issue a separate exact active packet only after owner authorization; the checklist and draft are not authorization.
9. Execute `runtime-to-review:guarded-live-probe` only with `RUNTIME_TO_REVIEW_V1_ONE_PROVIDER_ONE_IMAGE`, the real bound owner runtime module, an explicit owner-provided VCPToolBox root, one provider/plugin/API call, one image max, owner-runtime scope alignment, and receipt/status sync.
10. Start production workflow only after provider output can enter artifact / review / audit and human decision records safely.
