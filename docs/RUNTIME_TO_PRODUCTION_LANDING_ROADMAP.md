...
- `runtime_to_review_v2_review_feedback_fresh_prompt_target_selection` 已补一个 Green metadata-only target selection：它把正式写入目标从既有 tracked v3 改为未来 fresh target `prompts/image_generation/product_lifestyle_premium_portable_led_camping_lantern_v11.yaml`，证明 v3 冲突、v11 当前不覆盖既有 prompt package，并继续保持 prompt file write/provider/image/memory/production 全部 false；formal v11 prompt package draft remains blocked until human acceptance of the prompt patch preview。
...
5. Use `runtime_to_review_v2_review_feedback_fresh_prompt_target_selection` as the future target guard: the selected future prompt path is `prompts/image_generation/product_lifestyle_premium_portable_led_camping_lantern_v11.yaml`, while formal v11 prompt package draft remains blocked until human acceptance of the prompt patch preview.
6. If the prompt patch preview is human-accepted, create the formal v11 prompt package as a separate exact-file, no-provider Green task.
