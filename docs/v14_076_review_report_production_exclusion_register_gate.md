...
```yaml
phase: v14_076_review_report_production_exclusion_register_gate
...
V14.076 extracts a ReviewReport production exclusion register from the
admission matrix. This makes `never_production` auditable as a register, not
only as a route label.

The register proves:

```text
all rejected candidates are registered as production exclusions
no pass candidate is registered as a permanent exclusion
unknown-failure rejected candidates are both memory-forbidden and never-production
exclusion removal is not allowed by this gate
no production candidate, accepted_samples write, memory write, provider call, plugin call, API call, or image generation occurs
```
...
```text
node --check scripts/validate_review_report_production_exclusion_register.js: passed
node scripts/validate_review_report_production_exclusion_register.js: passed
...
next_scope_limit: local_review_report_memory_admission_register_only_no_runtime_no_provider_no_plugin_no_api_no_image_no_memory_write_no_accepted_samples
