...
- Product image A5 path 已进入 repeated quota/rate-limit stop；Native Doubao static syntax / sandbox 已硬化；v7.250 在 Route 3（继续停止生成）下定义审片记录模板与状态流。下一步不自动重试生成，继续做静态审片台验收清单。
